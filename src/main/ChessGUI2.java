...
public class ChessGUI2 extends JFrame {
	...
	public void buildBoardPanel() {
		...
		for (int x = 0; x < squares.length; x++) {
			...
			if (x == 1 || x == 3 || x == 5 || x == 7 || x == 8 || x == 10 || x == 12 || x == 14 || x == 17 || x == 19
					|| x == 21 || x == 23 || x == 24 || x == 26 || x == 28 || x == 30 || x == 33 || x == 35 || x == 37
					|| x == 39 || x == 40 || x == 42 || x == 44 || x == 46 || x == 49 || x == 51 || x == 53 || x == 55
					|| x == 56 || x == 58 || x == 60 || x == 62) {
		...
		public void actionPerformed(ActionEvent e) {
			...
			whiteTurnChecks: if (whiteTurn) {
				...
				if ((pieces[position].charAt(0) == 'W') && (pieceHighLighted == true)
						&& (position != highlightedPiecePosition)) {
					squares[highlightedPiecePosition].setBackground(squareColor);
			...
			blackTurnChecks: if (whiteTurn == false) {
				if ((pieceHighLighted == true) && (position == highlightedPiecePosition)) {
					squares[highlightedPiecePosition].setBackground(squareColor);
	...
	public void highlightMoves(int p, String movePString) {
		...
		if (whiteTurn) {
			...
			case 'N':
				...
				break;
			case 'B':
				if (!absoluteCheck && (!isPinned || (isPinned && directionPinnedFrom.containsKey(true)))) // if not
																											// pinned or
																											// pinned on
																											// diag
				{
				...
				break;
			case 'R':
				if (!absoluteCheck && (!isPinned || (isPinned && directionPinnedFrom.containsKey(false)))) // if not
																											// pinned or
																											// pinned on
																											// horiz
				{
	...
	private boolean[] BPTaken = { false, false, false, false, false, false, false, false };
	private boolean[] BNTaken = { false, false };
	private boolean[] BBTaken = { false, false };
	private boolean[] BRTaken = { false, false };
	private boolean[] WPTaken = { false, false, false, false, false, false, false, false };
	...
	public boolean getDirectionPinnedFrom() {
		boolean pinnedOnDiag = false;

		for (Map.Entry<Boolean, Integer[]> entry : directionPinnedFrom.entrySet()) {
			pinnedOnDiag = entry.getKey();
		}

		return pinnedOnDiag;
	}

	public Integer[] getDirectionPinnedFromCaseNums() {
		Integer[] caseNums = new Integer[2];
